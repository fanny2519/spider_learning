...
class LianjiaSpider_hf(scrapy.Spider):
    name = "lianjia_hf"
    ...
    def start_requests(self):
        for page in range(1, 101):
            yield scrapy.Request(url="https://hf.lianjia.com/ershoufang/pg" + str(page), callback=self.parse)
            time.sleep(1)
    ...
    def parse_detail(self, response):
        ...
        if len(list_items) >= 13:
            for item in range(13):
                ...
                # print(house_title2[item])
                # print(describes_processed)
                # print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++=")
        elif len(list_items) < 13:
            for item in range(len(list_items)):
                # house_title = item.css('span.label::text').extract_first()
